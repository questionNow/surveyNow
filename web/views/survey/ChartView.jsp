...
<body>
	...
	<p align="center"
		style="font-family: 'Jua', sans-serif; font-size: 50px;"><%=dsList.get(0).getS().getsNum()%>.<%=dsList.get(0).getS().getsTitle() %></p>
	<h2 align="right"
		style="font-family: 'Jua', sans-serif; font-size: 30px;">
		설문 기간 :
		<%=dsList.get(0).getS().getsStartDate()%>
		~
		<%=dsList.get(0).getS().getsEndDate() %></h2>
	<br>
	<br>
	<div id="chartArea">
		<div id="Area"></div>
...
// ** 문제 갯수마다 piechart"넘버" 로 DIV를 추가해줌
  $("#chartArea").append("<div id='piechart<%=i%>' style='width: 900px; height: 400px;'></div>");  
	$("#Area").append("<div id='radio_area<%=i%>'></div>");
	$("#Area").append("<div id = 'bar_area<%=i%>'style='width: 500px; height: 400px;'></div>");
