			...
			<form id="submitSurvey">
				...
				<div id="survey">
					...
					<p id  = pstyle>카테고리 <select id = sCategory name="sCategory">
						...
						document.querySelector("#target0 > select")
							...
							<% for(int i  = 0 ; i < dsList.get(0).getSt().getTargetType().length ; i++) { %>	
								<div id = "target<%= i %>">
									...
									<select class = 'target<%= i %> target' name = targetType onchange = 'checkTarget(<%= i %>);'>
										...
										<script type="text/javascript">
										$(function(){
											<% for(int j = 0 ; j< dsList.get(0).getSt().getTargetType().length ; j++){%>
											$(".target<%= j%> option[value='<%= dsList.get(0).getSt().getTargetType()[j] %>']").attr("selected", true);
											$("<%= dsList.get(0).getSt().getTargetDetail().get(i)[j] %>").attr("checked", true);	
										    <% } %>
										});
