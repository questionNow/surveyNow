...
public class MyPointServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			int listCount = Integer.parseInt(request.getParameter("pNum"));
		...
		PageInfo pointpi = new PageInfo(currentPage, listCount, limit, maxPage, startPage, endPage);
		
		// 게시판 리스트 조회해오기
		ArrayList<Point> pointlist = new MyPageService().selectPointList(currentPage, limit, listCount);
		
		RequestDispatcher view = null;
		if(pointlist != null) {
			view = request.getRequestDispatcher("views/mypage/MyPagePointList.jsp");
			request.setAttribute("pontlist",pointlist);     // 현재 페이지에 화면에 뿌려질 게시글이 담긴 객체
			 request.setAttribute("pointpi",pointpi);		   // 페이지에 관련된 정보가 담긴 객체
		}else {
