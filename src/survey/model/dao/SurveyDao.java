...
//		System.out.println(userId);
		try {
			pstmt = conn.prepareStatement("SELECT * FROM SURVEY WHERE SUSERID =? AND SSTATUS ='H' ORDER BY SCREATEDT");
			pstmt.setString(1, userId);

			rs = pstmt.executeQuery();
			sList = new ArrayList<Survey>();
			while (rs.next()) {
				// sql형 Date를 util로 바꿨다가 원하는 포맷의 String형으로 바꿔서 Survey에 실어줌
				java.util.Date utilDate = new java.util.Date(rs.getDate("screatedt").getTime());
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				String utilD = sdf.format(utilDate);
				...
				Survey s = new Survey(rs.getInt("snum"), rs.getString("stype"), rs.getString("stitle"),
						...
						rs.getDate("screatedt"), rs.getString("suserId"), utilD);
	...
	public int updateHoldSurvey(Connection conn, Survey s) {
		PreparedStatement pstmt = null;
		int result = 0;
		try {
